import { Storage } from "@google-cloud/storage";

const key = {
  type: "service_account",
  project_id: "xzist-software",
  private_key_id: "f33513c85ecc45d60fb2b89bbe2e8da6bfb9a0aa",
  private_key:
    "-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----\n",
  client_email: "test-storage-acc@xzist-software.iam.gserviceaccount.com",
  client_id: "103388277809269315935",
  auth_uri: "https://accounts.google.com/o/oauth2/auth",
  token_uri: "https://oauth2.googleapis.com/token",
  auth_provider_x509_cert_url: "https://www.googleapis.com/oauth2/v1/certs",
  client_x509_cert_url:
    "https://www.googleapis.com/robot/v1/metadata/x509/test-storage-acc%40xzist-software.iam.gserviceaccount.com",
  universe_domain: "googleapis.com",
};

export const config = {
  api: {
    bodyParser: false,
  },
};

export default async function (req, res) {
  console.log("STARTING================");

  try {
    const storage = new Storage({
      credentials: key,
      projectId: "xzist-software",
    });

    const bucketName = "xzist_test_bucket_0";
    // const destination = `uploads/${filename}`; // File path in the bucket

    const bucket = storage.bucket(bucketName);

    // const file = bucket.file("test/" + req.query.file);
    const file = bucket.file(req.query.file);
    const options = {
      expires: Date.now() + 1 * 60 * 1000, //  1 minute,
      fields: { "x-goog-meta-test": "data" },
    };

    console.log("doing something....");
    console.log(req.query.file);

    const [response] = await file.generateSignedPostPolicyV4(options);
    res.status(200).json(response);
  } catch (error) {
    console.log("There has been an error!!");
    res.json(error);
  }
}

// export default async function (req, res) {

//   try {
//     const storage = new Storage({
//       credentials: key,
//       projectId: "xzist-software",
//     });

//     console.log("here");

//     console.log("here 2");

//     // This will be called for each file
//     busboy.on("file", (fieldname, file, filename, encoding, mimetype) => {
//       console.log("Received file " + filename);

//       const saveTo = path.join(os.tmpdir(), filename);
//       file.pipe(fs.createWriteStream(saveTo));

//       file.on("end", async () => {
//         const bucketName = "xzist_test_bucket_0";
//         const destination = `uploads/${filename}`; // File path in the bucket

//         // Uploads a local file to the bucket
//         await storage.bucket(bucketName).upload(saveTo, {
//           destination,
//         });

//         // After upload, remove the local file to free up space
//         fs.unlink(saveTo, (err) => {
//           if (err) console.error("Couldn't remove temp file", err);
//         });
//       });
//     });

//     busboy.on("finish", () => {
//       console.log("Done parsing form!");
//       res.status(200).json({ message: "file uploaded" });
//     });

//     req.pipe(busboy);
//   } catch (error) {
//     res.json(error);
//   }
// }

// export default async function (req, res) {
//   const storage = new Storage({
//     // keyFilename: "../../service_accounts/xzist-software-f33513c85ecc.json",
//     credentials: key,
//     projectId: "xzist-software",
//   });

//   try {
//     console.log("here is the storage");
//     const bucketName = "xzist_test_bucket_0";

//     const bucket = storage.bucket(bucketName);

//     const [files] = await bucket.getFiles();

//     console.log("Objects in the bucket:");
//     files.forEach((file) => {
//       console.log(file.name);
//     });

//     storage.getBuckets().then((x) => console.log(x));

//     let resBucket = await storage.getBuckets();

//     res.json({
//       success: true,
//       buckets: resBucket,
//       message: "...",
//       //   `Bucket ${bucketName} created.`,
//     });
//   } catch (error) {
//     console.log("error has occurred");
//     console.log(error);
//     res.json(error);
//   }
// }
