  ...
  return (
    <>
      <PageBreadcrumb
        title="Calendar"
        name="Calendar"
        breadCrumbItems={["XZIST", "Tasks"]}
      />
      <div className="card flex-1 p-4">
        {/* Search bar aswell */}

        <div className="flex items-center justify-between">
          <h1 className="card-title text-3xl font-bold text-main">Tasks</h1>
          ...
          <div className="p-4 px-12 flex flex-col shadow-md">
            ...
            <div className="flex flex-col gap-2">
              {taskCategories.map((c) => (
                <h3
                  style={{
                    color: selectedCategory === c.name ? "#ABC502" : "",
                  }}
                  className={`text-xl text-gray-500 cursor-pointer`}
                  onClick={() => {
          ...
          <div className="flex-1 flex flex-col gap-4">
            {filteredTasks &&
              filteredTasks.map((task, i) => {
                if (!task.completed) return <Task task={task} key={i} />;
              })}
          </div>
        </div>
      </div>
...
function Task({ task }) {
  ...
  // todo: open task if url has open

  useEffect(() => {
    const checkTaskModalOpen = async () => {
      try {
        console.log(router.query);
        const { taskToOpen } = router.query;

        if (taskToOpen === task._id) setIsOpen(true);
      } catch (error) {
        console.log(error);
      }
    };

    if (router.isReady) checkTaskModalOpen();
  }, [router.isReady]);
